...
{/* ------------------------------------------------------- */}
          <div className='border p-5  rounded-2xl mb-5 '>
                  <img className='w-96 h-[250px] rounded-xl' src={card.recipe_image} alt=""/>
                  <h3 className='text-2xl font-bold mt-3 underline'>{card.recipe_name}</h3>
                  <p className='text-sm mb-3  mt-3'>{card.short_description_recipe}</p>
                  <hr />
                  <p className='text-xl font-semibold mt-3 mb-3'>Ingredients:{card.recipe_ingredients} -</p>
                            <ul className='ml-32 text-lg font-semibold'>
                                <li>1. {card.ingredients_one}</li>
                                <li>2. {card.ingredients_two}</li>
                                <li>3. {card.ingredients_three}</li>
                            </ul>
                  <hr />
                  <div className='flex justify-around'>
                          <div className='flex items-center gap-1 mt-2'>
                          <IoTimeOutline />
                          <button>{card.recipe_preparing_time}</button>
                          </div>
                          <div className='flex items-center gap-1 mt-2'>
                          <ImPower />
                          <button>{card.calories} Calories</button>
                          </div>
                          
                  </div>
                  <button onClick={() => handelAdd(card)}  className='bg-red-300 px-4 py-2 rounded-2xl mt-5 font-semibold'>Want To Cook!</button>
                  <div>
                <button className='flex items-center gap-2 bg-yellow-200 px-3 py-2 mt-3 rounded-2xl font-bold'>
                         <GiVibratingShield />
                                                           Rating : {card.rating}/5</button>
            </div>
          </div>
{/* --------------------------------------------- */}
